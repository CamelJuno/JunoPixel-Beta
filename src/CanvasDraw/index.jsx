...
import { DefaultState, DrawingState } from "./interactionStateMachine";
...
export default class CanvasDraw extends PureComponent {
  ...
  handleDrawEnd = (e) => {
    if (!this.shouldDraw || this.props.disabled) {
      this.interactionSM = new DefaultState();
      return;
    }
    this.interactionSM = new DrawingState();
  ...
  simulateDrawingLines = ({ lines, immediate }) => {
    ...
    lines.forEach((line) => {
      ...
      if (immediate) {
        ...
        this.points = points;
        this.saveLine({
          brushColor: brushColor ? brushColor : "#000",
          brushRadius,
        });
        return;
      }
  ...
  drawPoints = ({ points, brushColor, brushRadius }) => {
    ...
    this.ctx.temp.moveTo(p2.x + 2, p2.y + 2);
    this.ctx.temp.beginPath();

    // we pick the point between pi+1 & pi+2 as the
    // end point and p1 as our control point
    var midPoint = midPointBtw(p1 + 2, p2 + 2);
